...
# ============================================================
# FEATURES (V4 IMPROVED — IMPORTANT PART)
# ============================================================

def extract_features(iq):

    # ------------------------------------------------------------
    # MODIFICATION 1: amplitude normalization (SNR invariance)
    # ------------------------------------------------------------
    amp = np.abs(iq)
    ...
    # ------------------------------------------------------------
    # MODIFICATION 2: LOCAL DYNAMICS (CRITICAL FIX from t-SNE)
    # → captures transitions instead of global stats only
    # ------------------------------------------------------------

    phase_second_derivative = np.mean(np.abs(np.diff(phase, 2)))
    freq_jitter_local = np.mean(np.abs(np.diff(inst_freq)))

    energy_local = np.mean(np.abs(np.diff(iq)))  # signal roughness

    # ------------------------------------------------------------
    # PSD FEATURES (kept but stable)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # SNR proxy (kept but no longer dominant)
    # ------------------------------------------------------------
    snr_proxy = np.mean(amp) / (np.std(amp) + 1e-12)
    ...
    return {
        ...
        # NEW CRITICAL FEATURES (from t-SNE fix)
        "freq_jitter": freq_jitter_local,
        "phase_accel": phase_second_derivative,
...
plt.title("t-SNE - SDR V4 Improved")
plt.legend()
plt.grid()
plt.show()
...
plt.title("Confusion Matrix - SDR V4")
plt.show()

# ============================================================
# SAVE MODEL (UNCHANGED FILES)
# ============================================================
