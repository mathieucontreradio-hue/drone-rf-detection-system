...
# ============================================================
# IMPERFECTIONS SDR (UNCHANGED - GOOD)
# ============================================================
...
def extract_features(iq):
    ...
    # ------------------------------------------------------------
    # MODIFICATION 1:
    # amplitude normalization → removes SNR dependency
    # (CRITICAL FIX for AM misclassification)
    # ------------------------------------------------------------
    amp = np.abs(iq)
    ...
    # ------------------------------------------------------------
    # MODIFICATION 2:
    # SNR-robust Welch still ok but kept consistent
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # MODIFICATION 3:
    # frequency stability metric → improves FSK vs GMSK separation
    # ------------------------------------------------------------
    freq_jitter = np.mean(np.abs(np.diff(inst_freq)))

    # ------------------------------------------------------------
    # MODIFICATION 4:
    # SNR proxy feature → helps model distinguish noise vs signal
    # but NOT used alone for modulation classification
    # ------------------------------------------------------------
    ...
    return {
        ...
        "flatness": flatness,
        "cyclo": cyclo,
        "freq_jitter": freq_jitter,   # NEW
        "snr_proxy": snr_proxy        # NEW
    }
...
for gen,label in classes:
    ...
    for _ in range(SAMPLES_PER_CLASS):
        ...
        # SDR REALISM PIPELINE (UNCHANGED)
        iq=agc(iq)
        iq=multipath(iq)
        iq=phase_noise(iq)
        iq=add_cfo_drift(iq)
...
plt.title("Confusion Matrix - SDR Robust V3")
plt.show()

# ============================================================
# SAVE MODEL (UNCHANGED NAMES)
# ============================================================
