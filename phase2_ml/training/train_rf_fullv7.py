...
# ============================================================
# CHANNEL (kept but controlled)
# ============================================================

def add_awgn(iq, snr_db):
    p = np.mean(np.abs(iq)**2)
    snr = 10**(snr_db/10)
    n = np.sqrt(p/snr/2) * (np.random.randn(len(iq)) + 1j*np.random.randn(len(iq)))
    return iq + n


def add_cfo(iq):
    t = np.arange(len(iq))
    f_off = np.random.uniform(-0.002, 0.002)
...
def gen_am():
    t = np.arange(N)/FS
    return (1+0.6*np.sin(2*np.pi*1000*t)) * np.exp(1j*2*np.pi*5000*t)
...
# ============================================================
# FEATURES (SNR-INVARIANT VERSION)
# ============================================================

def extract_features(iq):

    # -------------------------
    # STRICT NORMALIZATION (KEY FOR SNR INVARIANCE)
    # -------------------------
    ...
    # -------------------------
    # TIME FEATURES (SNR ROBUST)
    # -------------------------
    var_amp = np.var(amp)
    kurt = kurtosis(np.real(iq))
    var_dphi = np.var(dphi)

    # -------------------------
    # SPECTRAL FEATURES (NORMALIZED PSD)
    # -------------------------
    f, psd = welch(iq, fs=FS)
    psd = psd / (np.sum(psd) + 1e-12)

    entropy = -np.sum(psd * np.log2(psd + 1e-12))
    spectral_centroid = np.sum(f * psd)

    # -------------------------
    # CYCLOSTATIONARY (VERY IMPORTANT FOR ROBUSTNESS)
    # -------------------------
...
# ============================================================
# DATASET BUILD (SNR-INVARIANT STRATEGY)
# ============================================================
...
for gen,label in classes:
    ...
    for _ in range(SAMPLES_PER_CLASS):
        ...
        # SAME CHANNEL DISTRIBUTION FOR ALL CLASSES (IMPORTANT)
...
df.to_csv("dataset_snr_invariant.csv", index=False)
...
plt.title("PCA - SNR INVARIANT DATASET")
plt.legend()
plt.grid()
plt.show()
...
plt.title("t-SNE - SNR INVARIANT DATASET")
plt.legend()
plt.grid()
plt.show()
...
plt.title("Confusion Matrix - SNR INVARIANT")
plt.show()
