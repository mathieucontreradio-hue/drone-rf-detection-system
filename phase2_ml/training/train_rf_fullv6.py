...
# ============================================================
# CHANNEL (SIMPLIFIED FOR SEPARABILITY)
# ============================================================
...
def add_cfo(iq):
    # SMALL CFO ONLY (important for realism but not destruction)
    t = np.arange(len(iq))
...
def extract_features(iq):
    ...
    # spectral centroid (IMPORTANT FOR FSK vs GMSK)
    spectral_centroid = np.sum(f * psd)

    # -------------------------
    # CYCLOSTATIONARITY SIMPLE
    # -------------------------
...
df.to_csv("dataset_v3_simplified.csv", index=False)
...
plt.title("PCA - SIMPLIFIED CHANNEL")
plt.legend()
plt.grid()
plt.show()
...
plt.title("t-SNE - SIMPLIFIED CHANNEL")
plt.legend()
plt.grid()
plt.show()
...
plt.title("Confusion Matrix - SIMPLIFIED CHANNEL")
plt.show()
